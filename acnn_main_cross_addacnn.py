...
parser.add_argument('--arch-ae-pretrain', default=False, type=str2bool)
parser.add_argument('--arch-ae-detach', default=False, type=str2bool)
parser.add_argument('--load-ae-path', default='no', type=str)


parser.add_argument('--arch-ae-back', default='ae_v2', type=str)
parser.add_argument('--arch-ae-back-pretrain', default=True, type=str2bool)
parser.add_argument('--load-ae-back-path', default='no', type=str)
...
parser.add_argument('--embedding-loss-function-back',default='mse',type=str)
...
def main():
    ...
    # 3-1 load model_select
    if args.arch_ae_pretrain :
    ...
    # 3-2 load model_select

    if args.arch_ae_back_pretrain : # defalt : true

        model_ae_back, _ = model_select(args.arch_ae_back)
        model_ae_back = nn.DataParallel(model_ae_back).cuda()

        checkpoint_path = os.path.join(args.load_ae_back_path, 'model_best.pth')
        state = torch.load(checkpoint_path)
        model_ae_back.load_state_dict(state['state_dict'])
    ...
    if args.arch_ae_pretrain == False:
    ...
    criterion_embedding_back = loss_function_select(args.embedding_loss_function_back)
    ...
    try:
        if args.train_mode:
            for epoch in range(lr_schedule[-1]):
                ...
                train(model_seg =model_seg, model_ae = model_ae,model_ae_back = model_ae_back,train_loader=train_loader, optimizer_seg=optimizer_seg,optimizer_ae=optimizer_ae,criterion_seg=criterion_seg,criterion_ae=criterion_ae,
                      criterion_embedding=criterion_embedding,criterion_embedding_back=criterion_embedding_back,epoch= epoch, logger=trn_logger, sublogger=trn_raw_logger)
...
def core_mode(core_mode='ChannelSE',bottom_seg=None,bottom_ae=None):
...
def loss_function_select(loss_function):
    ...
    if loss_function == 'bce':
        ...
        dice = DiceLoss().cuda()
        criterion = ClDice(bce,dice,alpha=1,beta=1).cuda()
    else:
...
def train(model_seg, model_ae, model_ae_back, train_loader, optimizer_seg, optimizer_ae, epoch, criterion_seg, criterion_ae,
                      criterion_embedding,criterion_embedding_back, logger, sublogger):
    ...
    embedding_losses_back = AverageMeter()
    ious = AverageMeter()
    dices = AverageMeter()
    acds = AverageMeter()
    asds = AverageMeter()
    ...
    for i, (input, target,_,_) in enumerate(train_loader):
        ...
        if args.arch_ae_pretrain == False:
            ...
            output_ae, bottom_ae ,_ = model_ae(noisy_batch_input)
        ...
        # add acnn embedding loss2

        _, bottom_seg_back,_ = model_ae_back(output_seg)
        _, bottom_ae_back, _ = model_ae_back(output_ae)


        loss_embedding_back = embedding_loss(args.embedding_loss_function_back, criterion_embedding_back, bottom_seg_back, bottom_ae_back,args.arch_ae_detach)

        beta = float(args.embedding_beta)
        loss_embedding_back = beta * loss_embedding_back

        total_loss = (loss_seg) + (loss_embedding) + loss_embedding_back

        print('loss_seg : ',loss_seg)
        print('loss_ae : ', loss_ae)
        print('alpha * loss_embedding : ', loss_embedding)
        print('beta *loss_embedding_back : ', loss_embedding_back)
        ...
        embedding_losses_back.update(loss_embedding_back.item(), input.size(0))
    ...
    logger.write([epoch, losses.avg,embedding_losses.avg,embedding_losses_back.avg, ious.avg, dices.avg,acds.avg,asds.avg])
