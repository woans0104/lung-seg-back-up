...
parser.add_argument('--style-direct',default='unidirectional',choices=['unidirectional','Bidirectional'],type=str)
...
def main():
    ...
    if args.val_size == 0:
        ...
        train_dataset_random = loader.CustomDataset(train_image_path, train_label_path, transform1,
                                                    arg_mode=args.arg_mode,
                                                    arg_thres=args.arg_thres, arg_range=args.arg_range,dataset=args.train_dataset)
        train_loader_random = data.DataLoader(train_dataset_random, batch_size=args.batch_size, shuffle=True,
                                              num_workers=4)
...
def train(model_seg, model_ae, train_loader,train_loder_random, optimizer_seg, optimizer_ae, epoch, criterion_seg, criterion_ae,
    ...
    for i, ((input, target,_,_),(_, target_random,_,_)) in enumerate(zip(train_loader,train_loder_random)):
        ...
        output_seg,bottom_seg,center_seg = model_seg(input)

        # autoencoder denoising
        # random target
        noisy_batch_input = make_noise_input(target_random, args.salt_prob)
        output_ae, bottom_ae, center_ae = model_ae(noisy_batch_input.cuda())
        ...
        #style_alpha = style_alpha*10
        print('style_alpha : ',style_alpha)



        loss_style = 0
        for j in range(len(bottom_seg)):
            loss_style_ = nn.MSELoss().cuda()
            if args.style_direct == 'unidirectional' :
                loss_style_layer = loss_style_(bottom_seg[j], bottom_ae[j].detach())
                #import ipdb; ipdb.set_trace()
            else :
                "Bidirectional"
                loss_style_layer = loss_style_(bottom_seg[j], bottom_ae[j])

            loss_style_layer = style_alpha * (loss_style_layer)
            loss_style += loss_style_layer

        loss_style = loss_style / len(bottom_seg)

        # embedding loss
        loss_embedding = embedding_loss(args.embedding_loss_function, criterion_embedding, center_seg, center_ae,
                                        args.arch_ae_detach)

        loss_style_add = loss_style + loss_embedding

        total_loss = (loss_seg) + (loss_style_add)
        ...
        embedding_losses.update(loss_style_add.item(), input.size(0))
