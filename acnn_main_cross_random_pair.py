...
Do not pair images and labels as inputs, 
but randomly sample and put labels into the autoencoder.
...
parser.add_argument('--load-arch', default='ae', type=str)
parser.add_argument('--padding-size', default=1, type=int)
parser.add_argument('--batchnorm-momentum', default=0.1, type=float)
parser.add_argument('--acnn-alpha', default=0.5, type=float)
parser.add_argument('--core-mode', default='gg', type=str)
...
def main():
    ...
    if args.val_size == 0:
        train_path, test_path = loader.make_dataset(args.train_dataset + '_dataset',
                                                    train_size=args.train_size, val_size=False, mode='test')
        ...
        train_dataset = loader.CustomDataset(train_image_path, train_label_path, transform1, arg_mode=args.arg_mode,
                                             arg_thres=args.arg_thres, dataset=args.train_dataset)
        train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4)

        #Organize images and labels differently.
        train_dataset_random = loader.CustomDataset(train_image_path, train_label_path, transform1, arg_mode=args.arg_mode,
                                             arg_thres=args.arg_thres, dataset=args.train_dataset)
        train_loader_random = data.DataLoader(train_dataset_random, batch_size=args.batch_size, shuffle=True, num_workers=4)
        ...
        train_path, val_path, test_path = loader.make_dataset(args.train_dataset + '_dataset',
        ...
        train_image_path = train_path[0]
        train_label_path = train_path[1]
        val_image_path = val_path[0]
        val_label_path = val_path[1]
        test_image_path = test_path[0]
        test_label_path = test_path[1]
        ...
        val_dataset = loader.CustomDataset(val_image_path, val_label_path, transform1, arg_mode=False,
                                           dataset=args.train_dataset)
        val_loader = data.DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=4)
    ...
    # 'MC'test_dataset
    test_data1_path,_ = loader.make_dataset(args.test_dataset1 + '_dataset', train_size=1,mode='test')
    test_data1_dataset = loader.CustomDataset(test_data1_path[0], test_data1_path[1], transform1,dataset=args.test_dataset1)
    test_data1_loader = data.DataLoader(test_data1_dataset, batch_size=1, shuffle=True, num_workers=4)

    # 'sh'test_dataset
    test_data2_path,_ = loader.make_dataset(args.test_dataset2+ '_dataset', train_size=1,mode='test')
    test_data2_dataset = loader.CustomDataset(test_data2_path[0], test_data2_path[1], transform1,dataset=args.test_dataset2)
    ...
    # 3.model_select
    model_seg, model_name = model_select(args.arch)
    model_ae, _ = model_select(args.load_arch)


    #checkpoint_path = os.path.join(args.load_model_path, 'model_best.pth')
    #state = torch.load(checkpoint_path)
    #net_load.load_state_dict(state['state_dict'])
    ...
    optimizer_seg = torch.optim.SGD(model_seg.parameters(), lr=args.initial_lr, momentum=0.9,
                                    weight_decay=args.weight_decay)
    optimizer_ae = torch.optim.SGD(model_ae.parameters(), lr=args.initial_lr, momentum=0.9,
                                   weight_decay=args.weight_decay)
    ...
    criterion_seg =loss_function_select(args.loss_function)
...
#####################################################################################
    ...
    try:
        if args.train_mode:
            for epoch in range(lr_schedule[-1]):
                ...
                train(model_seg =model_seg, model_ae = model_ae,train_loader=train_loader,train_loder_random=train_loader_random, optimizer_seg=optimizer_seg,optimizer_ae=optimizer_ae,criterion_seg=criterion_seg,criterion_ae=criterion_ae,
                      criterion_embedding=criterion_embedding,epoch= epoch, logger=trn_logger, sublogger=trn_raw_logger,acnn_alpha=args.acnn_alpha)

                iou = validate(model=model_seg, val_loader= val_loader,criterion = criterion_seg, epoch= epoch,logger= val_logger,save_fig=False,work_dir_name='{}_visualize_per_epoch'.format(args.train_dataset))
                #iou = validate(val_loader, model_seg, criterion_seg, epoch, val_logger,save_fig=False,work_dir_name='{}_visualize_per_epoch'.format(args.train_dataset))
                print('mc_iou **************************************************************')
                test_val_logger = Logger(os.path.join(work_dir, '{}_validation.log'.format(args.test_dataset1)))
                valid_iou = validate(model = model_seg,val_loader=test_data1_loader,criterion= criterion_seg,epoch= epoch,logger= test_val_logger, save_fig=False,
                                     work_dir_name='{}_visualize_per_epoch'.format(args.test_dataset1))
    ...
    #draw_curve_3(work_dir, trn_logger, val_logger,mc_val_logger)
...
def model_select(network):
    ...
    if network == 'unet' or network == 'acnn_decoder':
        my_net = Unet2D(in_shape=(1, 512, 512), padding=args.padding_size, momentum=args.batchnorm_momentum)

    elif network == 'unetcoord':
        print('radious', args.radious, type(args.radious))
        my_net = Unetcoordconv(in_shape=(1, 512, 512), padding=args.padding_size,
                            momentum=args.batchnorm_momentum, coordnumber=args.coordconv_no, radius=args.radious)

    elif network == 'ae':
        my_net = autoencoder()

    elif network =='ae_v2' or network == 'SRM':
        my_net= autoencoder_v2()


    #check without relu
    elif network =='unet_relu':
        my_net= Unet2D_check_relu(in_shape=(1, 512, 512), padding=args.padding_size, momentum=args.batchnorm_momentum)
    elif network =='ae_relu':
        my_net= autoencoder_v2_check_relu()
...
def train(model_seg, model_ae, train_loader,train_loder_random, optimizer_seg, optimizer_ae, epoch, criterion_seg, criterion_ae,
                      criterion_embedding, logger, sublogger, acnn_alpha):
    ...
    for i, ((input, target,_,_),(_, target_random,_,_)) in enumerate(zip(train_loader,train_loder_random)):
        ...
        if args.denoising == True:
            # random target
            noisy_batch_input=make_noise_input(target_random, args.salt_prob)
            output_ae, bottom_ae = model_ae(noisy_batch_input)

        else:
            output_ae, bottom_ae = model_ae(target_random)


        loss_seg = criterion_seg(output_seg, target)
        loss_ae = criterion_ae(output_ae, target_random)


        if args.core_mode == 'ChannelSE':
            bottom_seg = F.avg_pool2d(bottom_seg, 16)
            bottom_ae = F.avg_pool2d(bottom_ae, 16)

        elif args.core_mode == 'SpatialSE':
            # 1x1
        ...
        if args.embedding_loss_function == 'kl':
            loss_embedding = criterion_embedding(F.log_softmax(bottom_seg), F.softmax(bottom_ae.detach()))
        elif args.embedding_loss_function == 'jsd':
            loss_embedding = (0.5*criterion_embedding(F.log_softmax(bottom_seg), F.softmax(bottom_ae.detach()))) + (0.5*criterion_embedding(F.log_softmax(bottom_seg),F.softmax(bottom_ae.detach())))
        elif args.embedding_loss_function == 'bce':
            bottom_ae = F.sigmoid(bottom_ae).cuda()
            loss_embedding = criterion_ae(bottom_seg, bottom_ae.detach())  # bce
        else:
            #MSE
            loss_embedding = criterion_embedding(bottom_seg, bottom_ae.detach())


        b =  float(acnn_alpha)
        total_loss = (loss_seg) + (b * loss_embedding)

        print('a * loss_seg : ',loss_seg)
        print('a * loss_ae : ', loss_ae)
        print('b * loss_embedding : ', b * loss_embedding)
...
def validate(model,val_loader, criterion, epoch, logger, save_fig=False, work_dir_name=False):
