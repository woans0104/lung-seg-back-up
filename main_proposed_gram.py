...
def main():
    ...
    # 2.model_select
    model_seg,_ = utils.select_model(args.arch_seg, style=args.style)
    model_ae, _ = utils.select_model(args.arch_ae, style=args.style)
        ...
        if args.train_mode:
            for epoch in range(lr_schedule[-1]):
                ...
                train(model_seg=model_seg, model_ae=model_ae, train_loader=train_loader, epoch=epoch,
                      criterion_seg=criterion_seg, criterion_ae=criterion_ae,
...
def train(model_seg, model_ae, train_loader, epoch, criterion_seg, criterion_ae,
    ...
    for i, (input, target,_,_) in enumerate(train_loader):
        ...
        for j in range(len(bottom_seg)):
            ...
            if args.style_direct == 'unidirectional' :
                seg_gram = utils.gramMatrix(bottom_seg[j])
                ae_gram = utils.gramMatrix(bottom_ae[j].detach())
                loss_style_layer = loss_mse(seg_gram,ae_gram)
            else :
                "Bidirectional"
        ...
        iou, dice = utils.performance(output_seg, target)
    ...
    if args.arch_seg == 'unet_recon':
        logger.write([epoch, losses.avg, recon_losses.avg, ious.avg, dices.avg])
    else:
