...
def main():
    ...
    model_disc, _ = model_select('disc')
    ...
    model_disc = nn.DataParallel(model_disc).cuda()
    ...
    optimizer_ae = torch.optim.SGD(model_ae.parameters(), lr=args.initial_lr, momentum=0.9,
                                   weight_decay=args.weight_decay)
    optimizer_disc = torch.optim.SGD(model_disc.parameters(), lr=args.initial_lr, momentum=0.9,
                                   weight_decay=args.weight_decay)
    # lr decay
    ...
    lr_scheduler_disc = optim.lr_scheduler.MultiStepLR(optimizer_disc,
                                                     milestones=lr_schedule[:-1],
                                                     gamma=args.gamma)
    ...
    criterion_adversarial = loss_function_select('bce') #bce
    ...
    try:
        if args.train_mode:
            for epoch in range(lr_schedule[-1]):
                ...
                train(model_segG =model_seg, model_ae = model_ae,model_D = model_disc,train_loader=train_loader, optimizer_seg=optimizer_seg,optimizer_ae=optimizer_ae,optimizer_D=optimizer_disc,criterion_seg=criterion_seg,criterion_ae=criterion_ae,
                      criterion_embedding=criterion_embedding, criterion_adversarial=criterion_adversarial,epoch= epoch, logger=trn_logger, sublogger=trn_raw_logger)
...
def model_select(network):
    ...
    elif network =='unet_srm' or network == 'SRM':
        my_net= Unet2D_srm(in_shape=(1, 256, 256))
    elif network =='ae_v2':
...
def embedding_loss(embedding_loss,criterion_embedding, bottom_seg, bottom_ae):
    if embedding_loss == 'kl':
        ...
        loss_embedding = (0.5 * criterion_embedding(F.log_softmax(bottom_seg), F.softmax(bottom_ae.detach()))) + (
                    0.5 * criterion_embedding(F.log_softmax(bottom_seg), F.softmax(bottom_ae.detach())))
    elif embedding_loss == 'bce':
        bottom_ae = F.sigmoid(bottom_ae).cuda()
        loss_embedding = criterion_embedding(bottom_seg, bottom_ae.detach())  # bce
    else:
        # MSE
...
def train(model_segG, model_ae,model_D, train_loader, optimizer_seg, optimizer_ae,optimizer_D, epoch, criterion_seg, criterion_ae,
                      criterion_embedding,criterion_adversarial, logger, sublogger):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    embedding_losses = AverageMeter()
    Dfake_losses = AverageMeter()
    Dreal_losses = AverageMeter()
    ...
    for i, (input, target,_,_) in enumerate(train_loader):
        ...
        #######################################################
        # (1) Update D network: minimize log(1-D(G(z))) (But do not train G)
        ######################################################

        # 1A: Train D with real data.

        # Learning real data = autoencoder
        ...
        y_real_pred  = model_D(bottom_ae.detach())
        errD_real = criterion_adversarial(y_real_pred , torch.ones_like(y_real_pred).cuda())
        errD_real.backward()


        # 1B: Train D with fake data.
        # Calculate the loss with fake images - segmentation

        output_seg, bottom_seg = model_segG(input)
        y_fake_pred  = model_D(bottom_seg.detach())
        errD_fake = criterion_adversarial(y_fake_pred , torch.zeros_like(y_fake_pred).cuda())
        errD_fake.backward()
        errD = errD_real + errD_fake
        optimizer_D.step()

        # Only the parameters of D are optimized. It is changed by the stored derivative value calculated by backward ().

        #######################################################
        # (2) Update G network: maximize log(D(G(z))) images but do not train G
        ######################################################

        # 2. Use D's response to train G. (But I don't train D with this label.)
        ...
        y_fake_pred  = model_D(bottom_seg)
        errG_D = criterion_adversarial(y_fake_pred, torch.ones_like(y_fake_pred).cuda()) # y_fake_pred , label =1
        loss_seg, loss_bce, loss_dice = criterion_seg(output_seg, target)

        # embedding loss
        loss_embedding = embedding_loss(args.embedding_loss_function,criterion_embedding, bottom_seg, bottom_ae)

        alpha =  float(args.embedding_alpha)
        loss_embedding = alpha * loss_embedding
        total_loss = (loss_seg) + (loss_embedding) + errG_D


        total_loss.backward()
        optimizer_seg.step() # Only G parameters are optimized.
        ...
        print('1 * loss_G : ', errG_D)
        print('1 * loss_D_real : ', errD_real)
        print('1 * loss_D_fake : ', errD_fake)
